...
import { ITable, IGameDelay } from "../../interface";

dotenv.config({ path: "../../../.env" });
const turnDelay = async (data: IGameDelay) => {
  try {
    let redisData: any = {
    ...
    let turnDelayQueue = new Queue(BULL_KEY.TURN_DELAY_QUEUE, {
      redis: redisData,
    });
    let option = {
    ...
    await turnDelayQueue.process(async (job: any) => {
      let tableData: ITable = await Get(
        `${REDIS_KEY.REDIS_TABLE}:${job.data.jobId}`
      );
      ...
      let validateUserTurnStartData = await userTurnStartValidation(
        userTurnStartData
      );
      Event.sendToRoom(job.data.jobId, validateUserTurnStartData);
    });
  } catch (error) {
