    ...
    if (data) {
      ...
      if (tableData) {
        ...
        if (!board[newRow][newCol]) {
          ...
          if (
            tableData.playerScore[0] == 12 ||
            tableData.playerScore[1] == 12
          ) {
            let winData: any = {
            ...
            Event.sendToRoom(socket.tableId, validateWinData);
            disconnect(socket);
          }
          let turnId =
            tableData.turnId == tableData.player[0]._id
          ...
          let validateUserTurnData = await userTurnStartValidation(
            userTurnStartData
          );
          Event.sendToRoom(tableData._id, validateUserTurnData);
        }
      }
    }
  } catch (error) {
