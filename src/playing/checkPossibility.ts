...
import { checkPossibilityValidation } from "../validation/requestValidation/checkPossibilityValidation";
import { resPossibilityValidation } from "../validation/responseValidation/resPossibilityValidation";

const checkPossibility = async (data: any, socket: any) => {
  try {
    data = checkPossibilityValidation(data);
    if (data) {
      ...
      if (tableData) {
        ...
        let validationPossibilityData = await resPossibilityValidation(
          possibilityData
        );

        validationPossibilityData
          ? Event.sendToSocket(socket.id, validationPossibilityData)
          : null;
      }
    }
  } catch (error) {
